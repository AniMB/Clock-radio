...
class IdleMode(ModeBase):
    ...
    def handle_refresh(self):
        if self.selecting_mode:
            ...
            self.show_time()
            # --- Alarm trigger logic --- Needs changes major ones
...
class FMMode(ModeBase):
    def __init__(self, encoder_pin, button_pin):
        ...
        self.volume=value_dict.get("volume")
        self.mute=value_dict.get("mute")

        # self.i2c = I2C(1, sda=Pin(4), scl=Pin(5), freq=100000)
        # self.radio = RDA5807M(self.i2c)
        # self.radio.set_frequency(self.freq)
        self.radio=Radio(self.freq, self.volume, self.mute)
    ...
    def sync_from_dict(self, value_dict):
        ...
        nowplaying = value_dict.get("nowplaying", 1)
        self.freq = value_dict.get(f"freq{nowplaying}", 100.0)
        self.radio.SetFrequency(self.freq)
        self._show()
    ...
    def cleanup(self):
        ...
        del self.radio  # or use self.radio.mute(True) / self.radio.power_off() if available
