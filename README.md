...
* **Dual rotary encoders** handle station tuning and volume (with press‑to‑mute).
...
* **Web dashboard** (Flask) lets you
  ...
  * Configure alarms with custom snooze interval.
  * Manage favourite station presets.
* **RTC module** (DS3231) keeps accurate time, powered via micro‑USB even when Pico is unplugged.
...
| 1   | DS3231 RTC module                   | Powered from micro‑USB  |
| 1   | 0.96" I²C OLED (SSD1306)            | 128×64 pixels           |
| 1   | TEA5767 FM tuner module             | I²C interface           |
| 2   | KY‑040 rotary encoders              | Tuning & volume         |
| 1   | Momentary push‑button               | Radio power             |
| 1   | SPST toggle switch                  | 12 h / 24 h selection   |
| 1   | 3.5 mm headphone jack + PAM8302 amp | Optional speaker out    |
...
│   ├── main.py               # Hardware control firmware (also reproduced here)
│   ├── json_handler.py       # Thread‑safe JSON read/write
│   ├── drivers/              # OLED, TEA5767, rotary encoder modules
│   └── …
├── web/                      # Flask dashboard
│   ├── app.py
│   ├── templates/
